<template>
  ...
  <div class="two fields">
    ...
    <div class="ui segment raised blue">
     <h4 class="ui large header">Payment</h4>
      <card class='stripe-card'
     :class='{ complete }'
     stripe='pk_test_AjjYCAe4gb9KcVYKSuy8Isr4'
  ...
  methods: {
    ...
    pay () {
      createToken().then((data) => {
        this.payment_data.token = data.token.id
        console.log(this.payment_data.token)
        MoltinService.checkout(this.checkoutData).then((order) => {
          console.log('Checking out')
          MoltinService.pay(order.data.id, this.payment_data).then(() => {
            console.log('Payment successful')
            MoltinService.deleteCart().then((response) => {
